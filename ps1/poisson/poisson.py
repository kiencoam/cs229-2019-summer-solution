...
def main(lr, train_path, eval_path, save_path):
    ...
    util.plot(y_eval, y_pred, save_path)
    # *** END CODE HERE ***
...
class PoissonRegression:
    ...
    def fit(self, x, y):
        ...
        for loop in range(self.max_iter):
            # Calculate gradient ascent direction
            d = np.zeros(x.shape[1])
            for i in range(x.shape[0]):
                d += (y[i] - np.exp(np.dot(self.theta, x[i]))) * x[i]
            d /= x.shape[0]

            # Calculate new theta
            new_theta = self.theta + self.step_size * d

            # Check whether problem already converges or not
