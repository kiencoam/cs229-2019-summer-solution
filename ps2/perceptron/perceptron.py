...
def predict(state, kernel, x_i):
    ...
    # *** START CODE HERE ***
    z = 0
    for i in range(len(state['beta'])):
        z += state['beta'][i] * kernel(x_i, state['x_train'][i])
    return sign(z)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    for i in range(len(state['beta'])):
        z += state['beta'][i] * kernel(x_i, state['x_train'][i])
    beta = learning_rate * (y_i - sign(z))
    state['beta'].append(beta)
    state['x_train'].append(x_i)
    # *** END CODE HERE ***
...
def train_perceptron(kernel_name, kernel, learning_rate):
    ...
    predicted_probability = np.mean(predict_y == test_y)
    print('Predicted probability with {} kernel: {}'.format(kernel_name, predicted_probability))
