...
def init_centroids(num_clusters, image):
    ...
    for i in range(num_clusters):
        centroids[i] = image[np.random.randint(H), np.random.randint(W)]
    return centroids
    # *** END YOUR CODE ***
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for i in range(max_iter):
        ...
        distances = np.linalg.norm(centroids.reshape(-1, 1, 3) - data.reshape(1, -1, 3), axis=2)
        clusters = np.argmin(distances, axis=0)
        for j in range(centroids.shape[0]):
            indices = np.where(clusters == j)[0]
            new_centroids[j] = np.mean(data[indices], axis=0)
