...
def initialize_mdp_data(num_states):
    ...
    # *** START CODE HERE ***
    mdp_data = {
        'value_func': np.random.uniform(low=0, high=0.10, size=num_states),
        'rewards': np.zeros(num_states),
        'transition_probabilities_0': np.full((num_states, num_states), 1 / num_states),
        'transition_probabilities_1': np.full((num_states, num_states), 1 / num_states),
        'transition_count_0': np.zeros((num_states, num_states)),
        'transition_count_1': np.zeros((num_states, num_states))
    }
    return mdp_data
    # *** END CODE HERE ***
...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    threshold = (np.dot(mdp_data['transition_probabilities_1'][state], mdp_data['value_func'])
                 - np.dot(mdp_data['transition_probabilities_0'][state], mdp_data['value_func']))
    if threshold > 0:
        return 1
    elif threshold < 0:
        return 0
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    # *** START CODE HERE ***
    mdp_data['rewards'][new_state] = reward
    if action == 1:
        mdp_data['transition_count_1'][state, new_state] += 1
    else:
        mdp_data['transition_count_0'][state, new_state] += 1
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for state in range(mdp_data['rewards'].size):
        if np.sum(mdp_data['transition_count_1'][state]) != 0:
            mdp_data['transition_probabilities_1'][state] = (mdp_data['transition_count_1'][state]
                                                             / np.sum(mdp_data['transition_count_1'][state]))
        if np.sum(mdp_data['transition_count_0'][state]) != 0:
            mdp_data['transition_probabilities_0'][state] = (mdp_data['transition_count_0'][state]
                                                             / np.sum(mdp_data['transition_count_0'][state]))
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    iterations_count = 1
    while True:
        new_value_func = (mdp_data['rewards'] + gamma *
                                  np.maximum(np.matmul(mdp_data['transition_probabilities_0'], mdp_data['value_func']),
                                             np.matmul(mdp_data['transition_probabilities_1'], mdp_data['value_func'])))
        if np.all(np.abs(new_value_func - mdp_data['value_func']) < tolerance):
            mdp_data['value_func'] = new_value_func
            break
        else:
            mdp_data['value_func'] = new_value_func
            iterations_count += 1
    return iterations_count == tolerance
    # *** END CODE HERE ***
